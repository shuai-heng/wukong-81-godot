...
func _initialize() -> void:
	...
	_assert_true(r4.contains("func _spawn_spell"), "R4 在 release 同帧覆盖 projectile 起点")
	...
	_assert_true(impact.contains("\"ward_tick\""), "E 持续护持有真实目标 Contact 小反馈")
	_assert_true(ward.contains("e.hit(tick_damage"), "E 保留旧 ward tick damage")
	_assert_true(ward.contains("_spawn_tick_contact(e.global_position"), "E tick 命中视觉发生在真实敌人位置")
