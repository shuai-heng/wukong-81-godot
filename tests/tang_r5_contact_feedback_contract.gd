...
func _init() -> void:
	...
	# R4 仍承担人物 Pose / palm 锚点；R5 不得退回 player.position + 固定偏移。
	...
	if r5.find("player.position + Vector2") >= 0 or r5.find("position + Vector2(30") >= 0:
		errors.append("R5 introduced a fixed fake anchor")
